...
export default {
    ...
    methods: {
        ...mapMutations('menuStore', ['setTagsList']),
        //打印表单页面
        btnPrint() {
                ...
                this.$confirm('页面数据未保存, 是否保存?', '提示', {
                    ...
                    if (this.$route.query.$temporaryUsername || this.$route.query['_blank']) {
                        ...
                        let exitIndex = this.tagsList.findIndex(c=>{
                            return c.sortpath === this.$route.path
                        })
                        this.tagsList.splice(exitIndex, 1);
        ...
        btnSave(button) {
            this.flowContext.operateName = button.text;
            this.flowContext.operateType = button.key;
            this.callBizFlowBtn(this.__flowOperate, 1);
