            ...
            <div class="conditon-bar">
                ...
                <ice-grid-layout :columns="4">
                    <ice-label :name="item.label"
                               v-for="(item,index) in query.filter(c=>c.type!='static')"
                ...
                <div class="export-operate">
                    ...
                    <div style="flex-grow: 1;display: flex;flex-direction: column">
                        <el-checkbox
                                ...
                                @change=" val=>{exportSelectedColumns = val ? showableColumns.map(item=>item.code) : []}">
                            全选
                        </el-checkbox>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            search(evt, remoteExport = false) {
                ...
                if (this.isFirstLoad) {//第一次进入页面
                    ...
                    this.query = this.exportQueryCopy.filter(item => item.type != 'tab' && typeof item.value !== 'function');
        ...
        computed: {
            ...
            showableColumns() {
                return this.exportColumn.filter(item => !item.hidden).filter(item => item.label);
            }
        },
        watch: {},
        mounted() {
            ...
            this.exportSelectedColumns = this.showableColumns.map(item => item.code)
        },
