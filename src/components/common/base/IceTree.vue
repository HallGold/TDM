    ...
    export default {
        ...
        data() {
            return {
                ...
                defaultExpand: this.defaultExpandKey,
                data: [],
                selected: ''
            }
        },
        ...
        methods: {
            ...
            load(node, resolve) {
                ...
                if (this.cusTree) {
                    ...
                    switch (level) {
                        case 1:
                            url = "/pms/Xminfo/listForJfjhByYear?year=" + node.data.cpname
                            break
                        case 2:
                            url = "/pms/PmsViewCpk/getByXmoid?xmoid=" + node.data.xmoid
                    }

                }
                if (node.level == 0) {//获取根节点
                    this.loadData(this.rootValue, resolve, true, url, node)
                } else {
                    this.loadData(node.data[this.valueProp], resolve, false, url, node)
                }

            },
            loadData(pid, resolve, isroot, url, node) {
                let params = {};
                params[this.treeParentProp] = pid;

                this.$axios.get(url, {params: params})
                    .then(result => {
                        if (this.cusTree && node.level == 2) {
                            result.data.map(c => {
                                c.leaf = true;
                            })
                        }
