<template>
  ...
  <div class="ice-container"
    ...
    <div class="conditon-bar"
         v-if="filterQuery.length>0&&queryModel=='normal'">
      <slot name="conditon-bar">

        <ice-grid-layout :columns="columnsNumber">
          <ice-label :name="item.label"
                     v-show="advanceQuery?true:index < columnsNumber"
            ...
            <ice-date-picker v-model="item.value"
                             v-if="item.type=='date'"
                             type="date"
                             :clearable="false"
                             v-bind="item"
    ...
    <div class="ice-button-bar"
      ...
      <slot name="search-bar">
        <el-button type="text"
                   :icon="advanceQuery?'el-icon-arrow-up':'el-icon-arrow-down'"
                   v-if="filterQuery.length>columnsNumber"
                   @click="advanceQuery=!advanceQuery">更多
        </el-button>
    ...
    <div class="ice-streak "
         v-if="filterQuery.length>0&&queryModel=='normal'"></div>
    <div class="ice-grid-tool-bar "
         v-if="buttons.length>0||(filterQuery.length>0&&queryModel=='quick')">
      ...
      <div class="ice-grid-button-bar"
        ...
        <slot name="operation-bar">
          <template v-for="button in buttons">
            ...
            <el-button type="primary"
                       :key="button.name"
                       @click="exportExcel(button.selectable)"
                       :ctrl-code="button.ctrlCode"
        ...
        <ice-dialog title="数据导出预览"
                    :visible.sync="exportSelectorShow"
                    height="700px"
                    width="1200px"
                    remounted>
      ...
      <div class="ice-quick-query"
           v-if="filterQuery.length>0&&queryModel=='quick'"
           :style="{width:quickQueryWidth?quickQueryWidth:'400px'}">
        <slot name="quick-query">
          <el-input :placeholder="quickQueryPlaceholder"
                    v-model="quickQuery"
                    :clearable="true"
                    maxlength="50"
            ...
            <el-button slot="append"
                       @click="advanceQuery=!advanceQuery"
                       v-if="hasAdvenceQuery"
                       unauth>高级查询
            </el-button>
            <!---->
          </el-input>
      ...
      <ice-drawer :width="400"
                  style="position: absolute"
                  v-model="advanceQuery"
                  v-if="queryModel=='quick'">
        <div style="display: flex;flex-direction: column;height: 100%">
          <div class="advance-query-title">
            <div class="title">高级查询</div>
            <i class="el-icon-close"
               style="cursor: pointer"
               @click="advanceQuery=false"></i>
          </div>
          ...
          <div class="main-content">
            <div class="ice-full-absolute">
              ...
              <vxe-table :data="gridDataList"
                         ...
                         :checkbox-config="hasCheckBox?{trigger:'row',checkMethod:checkMethod,highlight: highlightShow, checkAll: checkAll}:null"
                         highlight-hover-row
                         :highlight-current-row="chooseItem == 'single'"
                         :span-method="spanMethod?data=>spanMethod({...data,data:gridDataList}):null"
                         @current-change="tableCurrentChangeHandler"
                ...
                <!-- dataLoadComplete是避免序号栏比其他栏位先加载出来，导致页面布局很奇怪  && dataLoadComplete -->
                <vxe-table-column v-if="gridIndex"
                                  fixed="left"
                                  ...
                                  width="36">
                  <template slot-scope="scope">
                    <span>{{scope.rowIndex + 1+(pageNum-1)*pageSize}}</span>
                  </template>
                </vxe-table-column>

                <vxe-table-column v-for="(item,index) in showableColumns"
                                  :key="item.code"
                                  :title="item.label"
                                  :field="item.code"
                                  :showOverflow="item.showOverflow?item.showOverflow:'title'"
                                  :label="null"
                  ...
                  <template slot-scope="scope">
                    <vxe-cell-render :field="item"
                                     :data="scope">
                    </vxe-cell-render>
                  </template>
                </vxe-table-column>

                <vxe-table-column v-if="operations.length>0"
                                  fixed="right"
                                  title="操作"
                  ...
                  <template slot-scope="scope">
                    <el-button @click.stop="columnClick(scope.row,scope.$index,operation.callback)"
                               ...
                               :key="operation.name"
                               v-if="operation.isShow?operation.isShow(scope.row,scope.$index):true">
                      <span :style="operation.styles">{{operation.name}}</span>
                    </el-button>
                  </template>
                </vxe-table-column>
...
export default {
  ...
  methods: {
    ...
    checkMethod ({ row }) {
      ...
      // 部门选择人员禁用
      if (this.disCode && this.disabledsCodes) {
        return (this.disabledsCodes.codes.indexOf(row[this.disCode]) == -1) || (this.disabledsCodes.oids.indexOf(row.oid) > -1) ? true : false
      }
      // 禁用回调方法
      if (this.cusCheckedMethod) {
        return this.cusCheckedMethod({ row });
      }
      return true;
    },
    getSelections () {
    ...
    closeSelection (value) {
      console.log(value, this.gridDataList, this.codeProp)
    ...
    // 设置选中状态 --By qwl
    setSectStatus () {
      if (this.sectData && this.sectData.length > 0) {
        this.$nextTick(() => {
          this.sectData.forEach((c) => {
            let item = this.gridDataList.find((p) => {
              return c.oid == p.oid;
            })
            if (item) {
