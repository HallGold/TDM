    ...
    <div class="ice-upload">
        <div v-if="doSecret" style="width: 100%">
            ...
            <div style="display: flex;align-items: stretch;width: 100%"
                ...
                <div v-if="!uploading&&filename" style="line-height: 30px;">
        ...
        <div v-if="!doSecret">
            <div class="ice-full-absolute" style="display: flex" v-if="uploaderDisabled&&value&&!$scopedSlots.default"
                ...
                <div>{{filename}}
                    <el-button style="margin-left: 10px" type="text"
                               @click="_=>{$downloadFile(value,false);$emit('downloadFile')}">下载
                    </el-button>
                </div>
            </div>
            <div class="ice-full-absolute" style="display: flex" v-if="uploaderDisabled&&!value&&!$scopedSlots.default"
            ...
            <div style="display: flex;position: relative">
                ...
                <el-input placeholder="请选择上传的文件" style="line-height: 30px;" v-model="filename" clearable
                    ...
                    <el-upload
                            ...
                            slot="append"
                            :limit="1">
                        <el-button @click="reset" :icon="uploading?'el-icon-loading':'el-icon-upload'" title="点击上传文件"
                                   unauth></el-button>
                    </el-upload>
                    <div slot="suffix" v-if="uploading">{{percent}}%</div>
                    <div slot="suffix" v-if="!uploading&&filename">
        ...
        <ice-dialog :visible.sync="visible" width="500px" title="附件上传" v-if="doSecret&&!uploaderDisabled">
            ...
            <ice-form-group :name="'附件(不能上传格式：'+noUpdateFiledsa.join('、')+')'" :expandable="false">
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapState('datamapStore', [ 'noUpdateFileds']),
            sectetText() {
        ...
        methods: {
            ...mapMutations('datamapStore', ['addUndoTypeCodes', 'addDataMapNoFile']),
            ...mapGetters('datamapStore', ['getDataMap', 'getReversedDataMap']),
            ...mapGetters('datamapStore', [ 'getDataUploadFiled']),
            onchange: function (file) {
                let suffixName = file.name.substr(file.name.lastIndexOf('.'));
                // 判断文件格式
                if (this.noUpdateFileds.indexOf(suffixName) >= 0) {
                    this.$message.error('上传文件格式错误！');
                    return
                }
                // 判断文件名字长度 限制在30个字符
                if (file.name.length>30) {
                    this.$message.error('文件名称太长，限制在30个字符');
                    return;
                }
                if (file.size<=0) {
                    this.$message.error('不能上传为0kb的文件！');
                    return
                }
                // 判断文件大小
                if (file.size > this.maxSize) {
                    return this.$message({
                        message: `您选择的文件大于${this.getSize(this.maxSize)}`,
                        type: 'error'
                    })
                }
                    ...
                    this.$axios
                        ...
                        .then(response => {
                            if (response.data) {
                                ...
                                this.$emit('fileData', response.data)
                            }
                        })
                        .catch(error => {
