        ...
        <div class="dates" ref="dates">
            <div class="dates-bg">
                <div class="week-row" v-for="week in currentDates">
                    <div class="day-cell" :style="{height: h+'px'}" v-for="day in week"
            ...
            <div class="dates-events">
                <div class="events-week" v-for="week in currentDates">
                    <div class="events-day" :style="{height: h+'px'}" v-for="day in week" track-by="$index"
                         :class="{'today' : day.isToday,
              'not-cur-month' : !day.isCurMonth}" @click.stop="dayClick(day.date, $event)">
                        <p class="day-number">{{day.monthDay}}</p>
                        <div class="event-box">
                            <p class="event-item"
                               v-for="(event, eventindex) in day.events" v-show="(eventindex+1) <= sum(day)"
                               :class="[classNames(event.cssClass), {
                  'is-start'   : isStart(event.start, day.date),
                  'is-end'     : isEnd(event.end,day.date),

                  }]"
                               @mouseover="eventOver(event,$event)"
                               @mouseleave="eventLeave(event,$event)"
                               @click="eventClick(event,$event)">
                                {{isBegin(event, day.date, day.weekDay)}}
                            </p>
                            <p v-if="day.events.length > sum(day)"
            ...
            <div class="more-events" v-show="showMore"
                ...
                <div class="more-body">
                    <ul class="body-list" style="padding-right: 20px;">
                        <li :class="[classNames(event.cssClass)]" v-for="event in selectDay.events"
                            class="body-item"
                            @click="eventClick(event,$event)">
                            {{event.title}}
                        </li>
...
<script type="text/babel">
    ...
    export default {
        ...
        methods: {
            ...
            sum (day, type) {
                if (type==1) {
                    return day.events.length - this.eventLimit
                } else {
                    return (day.events.length - this.eventLimit)==1?(this.eventLimit+1):this.eventLimit;
                }
            ...
            selectThisDay(day, jsEvent) {
                console.log(day, 'seelctday')
                let arr = JSON.parse(JSON.stringify(day));
                arr.events = arr.events.slice(this.eventLimit);
                this.selectDay = arr;
                this.showMore = true;
                let pos = this.computePos(event.target);
                this.morePos.left = pos.left < 0 ? 0 : pos.left;
                this.morePos.top = pos.top > (this.calHeight - 300) ? pos.top - 300 : (pos.top-100);
            ...
            computePos(target) {
                ...
                return {
                    left: eventRect.left - pageRect.left - 100,
                    top: eventRect.top + eventRect.height - pageRect.top + 120
                }
            },
            ...
            eventLeave(event, jsEvent) {
                ...
                this.$emit("eventLeave", "leave");
            },
            eventOver(event, jsEvent) {
                // if (!event.isShow) {
                //     return
                // }
                ...
                this.timer = setTimeout(() => {
                    this.$emit('eventOver', event, jsEvent, pos);
                    clearTimeout(this.timer);
                    this.timer = null;
                }, this.t);
            },
            ...
            .more-events {
                ...
                .more-body {
                    ...
                    .body-list {
                        ...
                        .bg2 {
                            background: #bb2aa3;
                            background: rgba(187, 42, 163, 0.5);
                        }
