    ...
    <div class="ice-container">
        ...
        <div style="height: 100%">
            <vxe-table border resizable
                        ...
                        <el-row :gutter="20">
                            ...
                            <el-col :span="8">
                                <el-tabs :tab-position="tabPosition" style="height: 200px;">
                                    <el-tab-pane label="问题描述">{{row.situation}}</el-tab-pane>
                                    <el-tab-pane label="故障排查">{{row.reason}}</el-tab-pane>
                ...
                <vxe-table-column field="wtCode" title="质量问题归零处理单编号" width="160px"></vxe-table-column>
                <vxe-table-column field="wtName" title="问题名称"></vxe-table-column>
                <vxe-table-column field="wtType" title="问题类别" :cell-render="{name: 'mapTypeCode', mapTypeCode: 'ZLWTGL_WTLB'}"></vxe-table-column>
        ...
        <wtgl-detail ref="detail" :to-flow="see"></wtgl-detail>
...
<script>
    ...
    import wtglDetail from './details/wtglDetail'
    ...
    export default {
        ...
        data() {
            return {
                ...
                query: [
                    {type: 'input', column: 'wtCode', label: '问题编号', exp: 'like', value: ''},
                    {type: 'input', column: 'wtName', label: '问题名称', exp: 'like', value: ''},
                    {type: 'select', column: 'wtType', label: '问题类别', exp: 'like', value: '', mapTypeCode: "ZLWTGL_WTLB"},
        ...
        methods: {
            refresh() {
                ...
                this.$axios.get("/pms/QisZlwt/list", {params: this.tablePage}).then(result => {
                    // console.log(result)
            ...
            initiationProcess(){
                this.$router.push("/qis/zlycbh/zlwtgl_flow")
            },
            ...
            async getBhgp({row, rowIndex}) {
                ...
                await this.$axios.get("/pms/QisCpBhg/listByOidWt", {
                    params: {
                        oidwt: row.oid,
                        current: 1,
                        size: 100,
            ...
            edit(row, rowIndex) {
                this.$router.push("/qis/zlycbh/zlwtgl_flow?dataId=" + row.oid)
            },
            see(row, rowIndex) {
                var dataId = row.businessDataId ? row.businessDataId : row.oid;
                this.$router.push("/qis/zlycbh/zlwtgl_flow?oid=" + row.oid + "&dataId=" + dataId)
            },
            // 查看
            showDetail (row) {
        ...
        watch: {
            $route(New, old) {
                if(New.path === '/qis/zlycbh/zlwtgl' && old.path === '/qis/zlycbh/zlwtgl_flow'){
                    this.refresh();
                }
            }
        },
