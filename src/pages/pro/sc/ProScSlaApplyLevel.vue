<template>
    <div style="width: 100%;height: 100%;background: white" id="outer">
...
<script>
    ...
    export default {
            ...
            return {
                ...
                query:
                    [
                        ...
                        {type: 'static', value: '2', code: 'lvType', exp: '='},
        ...
        methods: {
             ...
             */
            initTemplate() {
                let width = document.getElementById("outer").offsetWidth-400;
                let arr = [
                    {label: '等级2', code: 'lv2', width: width/(this.lv+2)},
                    {label: '等级3', code: 'lv3', width: width/(this.lv+2)},
                    {label: '等级4', code: 'lv4', width: width/(this.lv+2)},
                    {label: '等级5', code: 'lv5', width: width/(this.lv+2)},
                    {label: '等级6', code: 'lv6', width: width/(this.lv+2)},
                    {label: '等级7', code: 'lv7', width: width/(this.lv+2)},
                ];
                this.columns = [
                    {label: '主键id', code: 'loid', hidden: true},
                    {code: 'criteriaId', hidden: true},
                    {label: '区域', code: 'areaName', width: width/(this.lv+2)},
                    {label: 'SLA指标', code: 'sname', width: width/(this.lv+2)},
                    {label: '等级1', code: 'lv1', width: width/(this.lv+2)},
                ];
            ...
            addCallback() {
                let url = "/pro/ProScSlaLevel/all?current=1&size=10000&lvType=2";
                this.$refs.proScLevel.openDialog(url,2);
            },
            addTargetCallback() {
                this.initEditTables();
                this.isAddFlag = true;
            ...
            initEditTables() {
                ...
                for (let i = 0; i < this.lv; i++) {
                    this.mainData.editTables.push({lv: i + 1, op: '', target: '', unit: '', lvType: 2});
                }
            },
            /**
             * 获取当前数据的等级
