...
<script>
    ...
    isSuccess() {
            ...
            handleNodeClick(data, node) {
                if(data == '0'){
                    this.$set(this.buttons[0],'disabled',true);//禁用
                }else{
            ...
            addCallback() {
                let url = "/pro/ProScSlaLevel/all?current=1&size=10000&lvType=1";
                this.$refs.proScLevel.openDialog(url, 1);
            },
            addTargetCallback() {
                this.initEditTables();
                this.isAddFlag = true;
            ...
            initEditTables() {
                ...
                for (let i = 0; i < this.lv; i++) {
                    this.mainData.editTables.push({lv: i + 1, op: '', target: '', unit: '', lvType: 1});
                }
            },
            /**
             * 获取当前数据的等级
