    ...
    <div class="BottomBox">
      ...
      <el-table :data="tableData"
        ...
        <el-table-column label="预约数量"
                         align="center"
                         prop="sampleNums"></el-table-column>
        ...
        <el-table-column label="收样仓库"
                         align="center"
                         prop="optionsValue">
          <template slot-scope="scope">
            <el-select v-model="scope.row.receiveSamplesWarehouse"
                       placeholder="请选择"
                       clearable
                       @change="selectChangeWarehouse"
                       v-if="scope.row.isDynamite==1?true:false">
              <el-option v-for="item in laboratoryOptions"
                         :key="item.oid"
                         :label="fatter(item)"
                         :value="item.oid">
...
export default {
  ...
  data () {
    return {
      ...
      dialogVisible: false,
      query: [
        { type: "input", label: "预约编号", code: "searchTerm", value: "" },
        // {
        //   type: "static", label: "", code: "singleNumber", value: () => {
        //     return this.form.SingleBumber
        //   }
        // },
        { type: "static", label: "", code: "type", value: 2 },
  ...
  methods: {
    ...
    save () {
       ...
       this.$axios.post('tdm/sample/warehousing', this.paramsData).then((res) => {
    ...
    /* 预约单号 */
    selectSingleBumber () {
      this.$axios.get('tdm/sample/getSampleLists', {
        params: {
          singleNumber: this.form.SingleBumber,
          type: 2,
        }
      }).then(res => {
