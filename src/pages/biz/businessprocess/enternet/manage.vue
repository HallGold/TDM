        ...
        methods: {
            ...
            initColumns() {
                ...
                this.columns = [
                    ...
                    {label: '设备责任人', code: 'dutyName', width: 100},
                    {
                        label: '设备责任人部门', width: 140, renderCell(h, data) {
                            return data.row.deptName + "[" + data.row.dutyOrgName + "]";
                        }
                    },
                    {
                        label: '设备名称', width: 150, code: "name"
                    },
                    {
                    ...
                    {
                        label: '设备状态', code: 'state', width: 80, renderCell(h, data) {
                            return _this.onStateRenderer(data.row.state);
                        }
                    },
                    {label: '保密编号', code: 'secretSn', width: 100},
                    {label: '设备编号', code: 'sn', width: 100},
                    {
                        label: '使用场所', width: 180, renderCell(h, data) {
                            return _this.onNetAndTypeRenderer(data.row.netarea, data.row.nettype) + "[" + data.row.currentplace + "]";
                        }
                    },
                    {label: '设备IP', code: 'masterIp', width: 100},
                    {label: '出厂编号', code: 'birthsn', width: 150},
                    {label: 'MAC地址', code: 'mac', width: 150},
                    {label: '申请人', code: 'afUserName', width: 120},
                    {label: '申请部门', code: 'afDepartmentName', width: 120},
                    {
                        label: '审批状态', code: 'afStatus', width: 100, renderCell(h, data) {
                            return _this.getNameByCode(_this.ENTER_ENUMS.STATE.properties, data.row.afStatus);
                        }
                    },
                    {label: '申请时间', code: 'afDate', width: 120},
                ];
            },
            /**
             * 初始化操作列
            ...
            isOperationShow(value) {
                if (value == this.ENTER_ENUMS.STATE.DRAFT) {//草稿状态
                    return true;
                }
                return false;
            },
