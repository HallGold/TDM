<template>
    <div class="content-filled">
        <ice-query-grid data-url="/biz/questionnaire/QuesPagers/list"
                        :query="query"
    ...
    export default {
        ...
        methods: {
            ...
            submitForm() {
                ...
                // this.$axios.post("/biz/questionnaire/QuesPublishInfo/publish", data);
                if (data) {
                    data.pagerId = this.pagerId;
                    data.afStatus = "2";
                    this.$axios.post("/biz/questionnaire/QuesPublishInfo/publish", data).then(_ => {
            ...
            async savePager() {
                ...
                const {data} = await this.$axios.post("/biz/questionnaire/QuesPagers/saveOrUpdate", {
                    $json: {
                        ...this.formData,
            ...
            async deletePager(pager) {
                ...
                try {
                    ...
                    const {data} = await this.$axios.delete("/biz/questionnaire/QuesPagers/del", {params: {id: pager.oid}})
            ...
            async editPager(pager) {
                let {data: pagerInfo} = await this.$axios.get("/biz/questionnaire/QuesPagers/get", {params: {id: pager.oid}})
            ...
            async examConfirm() {
                ...
                if (data.oid) {//直接保存
                    try {
                        const result = await this.$axios.post("/biz/questionnaire/QuesExamRepository/saveOrUpdate", data)
                    } catch (e) {
            ...
            async editExam(data, index) {
                if (data.examId) {//已保存
                    let result = await this.$axios.get("/biz/questionnaire/QuesExamRepository/get", {params: {id: data.examId}})
                    data = result.data;
                }
                this.editExamVisible = true
                this.$nextTick(_ => {
                ...
                switch (this.examSelectAction) {
                    ...
                    case 'add3'://拷贝题目，只删除oid
                        if (selections && selections.length > 0) {
                            ...
                            this.examSelectVisible = false;
                            if (item.oid) {
                                let {data} = await this.$axios.get("/biz/questionnaire/QuesExamRepository/get", {params: {id: item.oid}})
                                this.editExamVisible = true
                                delete data.oid;
