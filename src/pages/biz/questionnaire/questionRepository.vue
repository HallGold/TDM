<template>
    <div class="content-filled">
        <ice-query-grid data-url="/biz/questionnaire/QuesExamRepository/list"
                        :query="query"
    ...
    export default {
        ...
        methods: {
            ...
            async editExam(row) {
                this.visible = true
                if (row.oid) {
                    let {data} = await this.$axios.get("/biz/questionnaire/QuesExamRepository/get", {params: {id: row.oid}})
                    this.$nextTick(_ => {
            ...
            async deleteExam(row) {
                try {
                    if (row.oid) {
                        ...
                        try {
                            await this.$axios.delete("/biz/questionnaire/QuesExamRepository/del", {params: {id: row.oid}})
