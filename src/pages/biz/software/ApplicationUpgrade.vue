                ...
                <el-form :model="bizData" status-icon :rules="rules" ref="bizForm" label-width="100px">
                    ...
                    <ice-grid-layout :columns="3" name="软件详情">
                        ...
                        <el-form-item label="发布者" prop="softwareInfo.publishAuthor">
                            <el-input v-model="bizData.softwareInfo.publishAuthor" placeholder="发布者"
                                      :disabled="flowScope.formReadonly"  maxlength="64"
                                      :showWordLimit="true"></el-input>
                        </el-form-item>
                        <el-form-item label="关键字" prop="keywords">
                            <el-input v-model="bizData.softwareInfo.keywords" placeholder="关键字之间用逗号间隔"   maxlength="128"
                        ...
                        <el-form-item label="文件大小" prop="softwareInfo.softSize">
                            <el-input v-model="bizData.softwareInfo.softSizeKb"
                                      :disabled="true"></el-input>
                        </el-form-item>
                        <el-form-item label="软件图片" prop="softwareInfo.softIconId">
                            <ice-image-uploader v-model="bizData.softwareInfo.softIconId" :width="150" :height="150"
                                                class="avatar-uploader" v-if="!flowScope.formReadonly"></ice-image-uploader>
                            <img v-if="flowScope.formReadonly" :src="$showImage(bizData.softwareInfo.softIconId)" class="avatar-uploader" />
                        </el-form-item>
                    ...
                    <ice-grid-layout :columns="3" name="授权信息" v-if="bizData.level!='SHARE'">
                        ...
                        <!--<el-form-item label="授权单位" prop="specialDept" label-width="80px"
                            ...
                            <el-input v-model="authDeptNamesText" :disabled="true"></el-input>
                        </el-form-item>-->
                        <el-form-item label="授权人员部门" prop="specialPerson" label-width="100px"
                                      v-if="bizData.level=='AUTH'">
                            <el-button icon="el-icon-edit-outline" style="color: #dd6161" @click="chooseItem" :disabled="flowScope.formReadonly">
                        ...
                        <!--<el-form-item label="授权单位" prop="specialDept" label-width="80px" layout="3"
                                      v-if="bizData.softwareInfo.softRegion==1">
                            <ice-dept-selector
                                    choose-item="multiple"
                                    ref="persionDept"
                                    :selected-dept="bizData.dynamicDeptTags"
                                    @select-confirm="selectDeptConfirm"
                                    @select-cannel="cancelSelectUser"
                                    :disabled="flowScope.formReadonly">
                            </ice-dept-selector>
                        </el-form-item>
                        <el-form-item label="授权人员" prop="specialPerson" label-width="80px" layout="3"
                                      v-if="bizData.level=='AUTH'">
                            <ice-persion-selector
                                    choose-item="multiple"
                                    ref="persionPop"
                                    :selected-persion="bizData.dynamicUserTags"
        ...
        data() {
            ...
            return {
                ...
                rules: {
                    ...
                    'softwareInfo.softVersion': [{required: true,whitespace: true, message: '请输入软件版本', trigger: 'blur'}],
                    ...
                    'softwareInfo.softRegion':[{required: true,whitespace: true, message: '请选择关键字', trigger: 'change'}]
                },
                quillOptions:{
                    placeholder:'请输入软件描述...',
                    modules: {
        ...
        methods: {
            ...
            loadPageData(call) {
                ...
                this.$axios.get("/biz/BizSoftwareInfo/gets", {params: {ids: ids}})
                    .then(result => {
                            let ormData = result.data[0];
                            let temp = common.copy({c:false, pros:['softName','fromYon','softIconId','publishAuthor','softRegion','keywords','classifyIdPath','classifyNamePath','softDescribe']
                            },{}, ormData)
                            Object.assign(this.bizData.softwareInfo,temp);
                        this.bizData.softwareInfo.softwareId = ormData.softwareId ? ormData.softwareId : ormData.oid;
                            this.bizData.level = ormData.classifyIdPath.split("_")[0];
                            this.bizData.classifyId = ormData.classifyIdPath.split('_').splice(1);
                            console.log(ormData.classifyIdPath+','+this.bizData.classifyId)
                            return this.$axios.get("/biz/BizSoftwareAuthUser/getAuth", {"params": {"softwareId": ormData.oid}});
                        }
                    ).then(result => {
                        console.log('--------------')
