<template>
    <div class="content-filled">
        <ice-query-grid
                ref="iceGrid"
                data-url="/biz/BizSoftwareAuditUpdateAf/listByLoginUser"
                :query="query"
                :columns="columns"
                :buttons="buttons"
...
<script>
    ...
    export default {
        ...
        data(){
            return{
                query: [
                    ...
                    {type: 'static', code: "updateType", exp: "=", value: 'UPGRADE'}
                ],
                columns:[
                    {code:"oid",hidden:true},
                    {label: '申请单号', code: 'afNo', width: 170},
                    {label: '软件名称', code: 'softName', width: 270},
                    {label: '申请原因', code: 'afReason', width: 360,align:'left'},
        ...
        methods:{
            ...
            /**查看*/
            lookItem(row){
                this.$router.push("/biz/software/ApplicationUpgrade?dataId="+row.oid);
            },
            /**编辑*/
            updataItem(row){
                this.$router.push("/biz/software/ApplicationUpgrade?dataId="+row.oid);
            ...
            deleteItem(row){
                this.$confirm('确定要删除吗', '提示', {
                    ...
                    this.$axios.delete("/biz/BizSoftwareAuditUpdateAf/del",{"params":{"id":row.oid}}).then(success=>{
