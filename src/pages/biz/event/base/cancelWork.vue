<template>
    <el-form :model="mainData" class="right" :rules="formRules" ref="form">
        <el-form-item label="取消原因:" label-width="85px" layout="2" prop="reason"><!-- placeholder="请选择.."  -->
            <ice-select v-model="mainData.reason"
                        map-type-code="cancel"
                        @change="$nextTick(()=>{$refs.form.validateField('reason',error=>{})})">
            </ice-select>
        </el-form-item>
        <el-form-item label="说明:" label-width="85px" prop="detail" layout="3" style="min-height: 40px">
            <el-input v-model="mainData.detail" type="textarea" rows="6" class="textarea">
            </el-input>
        </el-form-item>
        <el-form-item class="ice-button-bar">
            <el-button type="primary" @click="confirmCancelWork">确定</el-button>
            <el-button type="info" @click="cancelCancelWork">取消</el-button>
        </el-form-item>
    </el-form>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            confirmCancelWork() {
                this.$emit("confirmCancelWork", this.mainData);
            },
            cancelCancelWork() {
                this.$emit("cancelCancelWork", false);
            }
        },
