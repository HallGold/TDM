<template>
    <div class="content-filled">
        <ice-query-grid title="应用系统更新管理"
                        :data-url="INSTITUTE_ENUMS.ACTIONS.SEARCH_BY_TYPE_CODE.URL()+'?typeCode='+this.INSTITUTE_ENUMS.SYSTEM_APPLY_TYPE_DATA.update.code"
                        :query="UPDATE_MANAGE_PAGE_ENUM.MANAGE_GRID.QUERY"
                        :columns="UPDATE_MANAGE_PAGE_ENUM.MANAGE_GRID.COLUMNS"
                        :ref="UPDATE_MANAGE_PAGE_ENUM.MANAGE_GRID.REF"
                        :operations="UPDATE_MANAGE_PAGE_ENUM.MANAGE_GRID.OPERATIONS"
                        :operationsWidth=100
                        :minHeight="530"
                        :buttons="UPDATE_MANAGE_PAGE_ENUM.MANAGE_GRID.BUTTONS"></ice-query-grid>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            initColumns() {
                let _this = this;
                this.UPDATE_MANAGE_PAGE_ENUM.MANAGE_GRID.COLUMNS = [
                    ...
                    {label: '使用单位', code: 'useDeptList', width: 150},
                    {label: '更新文件来源', code: 'updateSource', width: 150},
                    {label: '主管部门', code: 'competentDeptName', width: 100},
                    {label: '承建单位', code: 'factoryNameList', width: 150},
                    {label: '预计更新开始时间', code: 'expectUpdateTime', width: 180},
                    {label: '预计更新结束时间', code: 'expectEndTime', width: 180},
                    {label: '申请单位', code: 'creatorDeptName', width: 100},
                    {
                        label: '程序包', width: 150, showTips: false, formatter: row => {
                            return _this.onFileRenderer(_this.ATTACHMENT_ENUMS.institute_bsb, row.bizReFileVos);
                        }
                    },
                    {
                        label: '部署文件', width: 150, showTips: false, formatter: row => {
                            return _this.onFileRenderer(this.ATTACHMENT_ENUMS.institute_bswj, row.bizReFileVos);
                        }
                    }
                ];
            },
            ...
            initQuerys() {
                let _this = this;
                this.UPDATE_MANAGE_PAGE_ENUM.MANAGE_GRID.QUERY = [
                    ...
                    {type: 'date', label: '预计更新开始时间(起)', code: 'expectUpdateTime', exp: ">"},
                    {type: 'date', label: '预计更新开始时间(止)', code: 'expectUpdateTime', exp: "<"},
                ];
            },
            /**
             * 初始化按钮
             ...
             */
            initOperations() {
                this.UPDATE_MANAGE_PAGE_ENUM.MANAGE_GRID.OPERATIONS = [
             ...
             */
            view(row) {
                this.$router.push(this.INSTITUTE_ENUMS.ROUTER.UPDATE_EDIT.URL() + "?dataId=" + row.oid + "&readOnly=true");
            }
        },
        mounted() {
